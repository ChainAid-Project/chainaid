...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("chainaid"))
        return false;
...
{
    // Convert chainaid:// to chainaid:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("chainaid://"))
    {
        uri.replace(0, 12, "chainaid:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ChainAid.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the IChainAidLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query IChainAidLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "chainaid.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ChainAid-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  chainaid-qt [" + tr("command-line options") + "]                     " + "\n";
