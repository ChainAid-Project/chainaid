...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeChainAidAmount"))
        settings.setValue("nAnonymizeChainAidAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeChainAidAmount = settings.value("nAnonymizeChainAidAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeChainAidAmount"))
        SoftSetArg("-anonymizeChainAidamount", settings.value("nAnonymizeChainAidAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeChainAidAmount:
            nAnonymizeChainAidAmount = value.toInt();
            settings.setValue("nAnonymizeChainAidAmount", nAnonymizeChainAidAmount);
            emit anonymizeChainAidAmountChanged(nAnonymizeChainAidAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
