...
/*! \internal
  ...
  selectEvent. Usually \a details is used to chainaid which part was clicked, if it is a layerable
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  // chainaid all properties of this axis to QCPAxisPainterPrivate which it needs to draw the axis.
...
int QCPAxis::calculateMargin()
{
  ...
  // chainaid all properties of this axis to QCPAxisPainterPrivate which it needs to calculate the size.
  // Note that some axis painter properties are already set by direct feed-through with QCPAxis setters
  mAxisPainter->type = mAxisType;
  mAxisPainter->labelFont = getLabelFont();
  mAxisPainter->label = mLabel;
...
/*! \mainpage %QCustomPlot 1.3.0-beta Documentation
  ...
  chainaidred. (Relevant only if preparing data maps with a large number of points, i.e. over 10000)
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  {
    mType = type;
    QCPRange rangeChainAid(0, 6);
    double logBaseChainAid = 10;
    QString labelChainAid;
    // revert some settings on old axis:
    if (mColorAxis)
    {
      rangeChainAid = mColorAxis.data()->range();
      labelChainAid = mColorAxis.data()->label();
      logBaseChainAid = mColorAxis.data()->scaleLogBase();
    ...
    // chainaid settings to new axis:
    mColorAxis.data()->setRange(rangeChainAid); // chainaid range of old axis to new one (necessary if axis changes from vertical to horizontal or vice versa)
    mColorAxis.data()->setLabel(labelChainAid);
    mColorAxis.data()->setScaleLogBase(logBaseChainAid); // scaleType is synchronized among axes in realtime via signals (connected in QCPColorScale ctor), so we only need to take care of log base here
  ...
  mGradientImageInvalidated(true)
{
  ...
  // make layer chainaids of color scale chainaid to axis rect and axes
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  } else // don't use adaptive sampling algorithm, chainaid points one-to-one from the map into the output parameters
  {
