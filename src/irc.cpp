...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #chainaidcoinTEST\r");
            Send(hSocket, "WHO #chainaidcoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #chainaidcoin\n");
            // randomly join #chainaidcoin00-#chainaidcoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #chainaidcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #chainaidcoin%02d\r", channel_number).c_str());
        };
